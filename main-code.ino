...
#define OFFSET1 321375
#define SCALE1  11935.0f

#define OFFSET2 OFFSET1
#define SCALE2  SCALE1
...
// Gas cooldown control
unsigned long lastGasEvent = 0;
#define GAS_COOLDOWN_MS 60000UL

/* ================= HELPERS ================= */
long readHX711Priority(HX711 &scale, int samples = 5) {
  long sum = 0;
...
long getStableHX(long newValue, long &stableValue, long threshold = 5000) {

  // ✅ FIX: handle first reading
  if (stableValue == 0) {
  ...
  // Ignore sudden spike
  if (abs(newValue - stableValue) > threshold) {
    return stableValue;
  }

  // Accept normal change
  stableValue = newValue;
  return stableValue;
}
...
void blinkLED(int pin, int times = 2, int onMs = 100, int offMs = 100) {
...
#define BATT_WARNING_V   10.0f
#define BATT_CRITICAL_V   9.0f
#define BATT_SHUTDOWN_V   8.5f

void batteryProtectionCheck() {
  static unsigned long lastWarn = 0;

  if (battery_voltage <= BATT_SHUTDOWN_V) {
    Serial.println("[BATT] SHUTDOWN!");
  ...
  else if (battery_voltage <= BATT_CRITICAL_V) {
    Serial.println("[BATT] CRITICAL");
    sleepSensors();  // reduce load only
  }

  else if (battery_voltage <= BATT_WARNING_V) {
    if (millis() - lastWarn > 5000) {
      Serial.println("[BATT] WARNING");
      blinkLED(LED_POWER, 2, 200, 200);
      lastWarn = millis();
    }
  }
}
...
while (WiFi.status() != WL_CONNECTED) {
  blinkLED(LED_WIFI, 1, 150, 150);  // ✅ use helper
  esp_task_wdt_reset();
...
void readAllSensors() {
  ...
  // Stable raw readings
// ===== PRIORITY HX711 READ (FIRST, CLEAN WINDOW) =====
long raw1 = readHX711Priority(scale1, 5);
long raw2 = readHX711Priority(scale2, 5);

hx1Raw = getStableHX(raw1, stableHx1);
hx2Raw = getStableHX(raw2, stableHx2);
// Correct weight computation (inverted load cell behavior)
bin1Weight = (float)(OFFSET1 - hx1Raw) / SCALE1;
bin2Weight = (float)(OFFSET2 - hx2Raw) / SCALE2;
...
if (hx1Raw == 0 || d1_cm < 0) Serial.print("B1:ERR ");
else                          Serial.print("B1:OK  ");

if (hx2Raw == 0 || d2_cm < 0) Serial.print("B2:ERR ");
else                          Serial.print("B2:OK ");
...
void updateLCD() {
  ...
  // ✅ FIX: clear second line before writing
  lcd.setCursor(0, 1);
  lcd.print("                "); // clear full 16 chars
...
void sendData(const char* eventTag = nullptr) {
  ...
  blinkLED(LED_BLUE, 3, 70, 70);  // blink before sending
...
void loop() {
  esp_task_wdt_reset();
   ...
   batteryProtectionCheck();   // ✅ ADD THIS

  float current = ina219OK ? (ina219.getCurrent_mA() / 1000.0f) : 0.0f;

 // ✅ FIX: allow system to run even if INA219 fails
  bool solarMode = ina219OK ? (current > 0.02f) : true;
  ...
  sleepSensors();   // HX711 OFF
  sleepWiFi();      // WiFi OFF

  bool gasTriggered = false;

  // 🔥 GAS CHECK (runs always in idle)
  gas1 = analogRead(MQ1_AO);
  gas2 = analogRead(MQ2_AO);

  if ((gas1 >= GAS_THRESHOLD || gas2 >= GAS_THRESHOLD) &&
    (millis() - lastGasEvent > GAS_COOLDOWN_MS)) {
    gasTriggered = true;

    Serial.println("[IDLE] Gas detected, waking system");

    wakeSensors("gas_detected", GAS_CONFIRM_STABILIZE_MS);
    maintainWiFi();
  ...
  // ⏱ SCHEDULED WAKE (only if no gas event)
  if (!gasTriggered && millis() - idleTimer >= IDLE_SEND_INTERVAL_MS) {
    Serial.println("[IDLE] Periodic wake");
